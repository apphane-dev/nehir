# BarutSRB/OmniWM#349 — "Hidden window bleeds into view (multi-monitor)" — Discovery
...
frames to monitor bounds" (**unmerged upstream**). PR BarutSRB/OmniWM#364's commit message names BarutSRB/OmniWM#349.
Sibling items in the bleed family: BarutSRB/OmniWM#235 (bleed **across workspaces**, see
`noop/20260616-omniwm-235-window-bleed-different-workspace.md`) and BarutSRB/OmniWM#364. This doc keeps to
the **multi-monitor / monitor-bounds** angle; BarutSRB/OmniWM#235's workspace-lifecycle mechanism is
cross-referenced only where root causes touch.

Scope of this doc: determine whether the multi-monitor bleed in BarutSRB/OmniWM#349 applies to nehir,
and whether PR BarutSRB/OmniWM#364's `clampVisibleFramesToMonitorBounds` is needed / safe to port.
...
> symptom BarutSRB/OmniWM#349 photographs (a parked/"hidden" window's strip rendering on the *other*
> monitor) and the symptom PR BarutSRB/OmniWM#364 guards against (a *visible* tiled frame overhanging into
> a neighbouring monitor) are both already prevented in nehir by an earlier, stronger
> mechanism than PR BarutSRB/OmniWM#364's clamp. It owns no repo action: PR BarutSRB/OmniWM#364 is not ported, and no
> invariant test is mandated here. The doc is retained for the upstream-symptom record and
> the bleed-family cross-reference (BarutSRB/OmniWM#235). It is the deduped survivor of three concurrent
...
- **The symptom BarutSRB/OmniWM#349 photographs — a parked/"hidden" window's strip rendering on the
  *other* monitor — and the symptom PR BarutSRB/OmniWM#364 guards against — a *visible* tiled frame
  overhanging into a neighbouring monitor — are both already prevented in nehir by an
  earlier, stronger mechanism than PR BarutSRB/OmniWM#364's clamp: a column whose rendered rect
...
- **PR BarutSRB/OmniWM#364's `clampVisibleFramesToMonitorBounds` does NOT exist in nehir**
...
- **BarutSRB/OmniWM#349 is a DISTINCT mechanism from nehir's stale-live-frame discoveries.** PR BarutSRB/OmniWM#364
...
- `Sources/Nehir/Core/Layout/Niri/NiriLayoutEngine+Animation.swift:228` — nehir's
  ...
  `Sources/OmniWM/Core/Layout/Niri/NiriLayoutEngine+Animation.swift` that PR BarutSRB/OmniWM#364 edits.
...
- `git log -S "overflowEdgeIntersectingNeighboringMonitor"` → present since `9a46877
  "Initial Nehir import"`. The guard is **nehir baseline, not a backport of PR BarutSRB/OmniWM#364**.
...
### 1. nehir's layout return — no clamp (this is where PR BarutSRB/OmniWM#364 would insert)
...
func calculateCombinedLayoutUsingPools(
    ...
    calculateLayoutInto(                          // :249
        ...
        …)
    return (framePool, hiddenPool)                 // :265  ← PR BarutSRB/OmniWM#364 inserts clamp here
}
```

PR BarutSRB/OmniWM#364 (NOT in nehir — `ffgrep clampVisibleFramesToMonitorBounds` is empty) adds, between
`:263` and `:265`:

```swift
// PR BarutSRB/OmniWM#364 — verbatim from https://github.com/BarutSRB/OmniWM/pull/364.diff
...
That comment describes the exact BarutSRB/OmniWM#349 failure mode and is nehir's deliberate guard. The
resolver scores each candidate side × vertical lane by `overlapArea` against **all**
monitors and commits the minimum-overlap origin. PR BarutSRB/OmniWM#364 does nothing for these windows —
...
1. **The visible-frame overhang is prevented before any visible frame is produced.** A
   ...
   PR BarutSRB/OmniWM#364's clamp would be a **no-op** in nehir's normal side-by-side multi-monitor
...
2. **The hidden-window strip (the literal report) is handled by a different path.**
   ...
   PR BarutSRB/OmniWM#364 explicitly **excludes** `hiddenPool` windows, so even in OmniWM it cannot address
...
The triage asked whether BarutSRB/OmniWM#349 is the SAME root cause as "a stale/live frame drawn for a
...
## Relationship to the bleed family (BarutSRB/OmniWM#235, BarutSRB/OmniWM#364)
...
- **BarutSRB/OmniWM#349 (this doc) / BarutSRB/OmniWM#364** — a **geometry** problem: a frame straddling a monitor
...
- **BarutSRB/OmniWM#235** — a **workspace visibility / lifecycle** problem (bleed *across workspaces*,
  ...
  ever shows a BarutSRB/OmniWM#235-style symptom, route it to workspace visibility work, not PR BarutSRB/OmniWM#364.

## Recommendation

- **Do not port PR BarutSRB/OmniWM#364.** Unmerged upstream, redundant with nehir's
...
- **Residual edge case to monitor:** if a hidden window is larger than every parking lane,
  ...
  could remain in an extreme case. PR BarutSRB/OmniWM#364 cannot help (hidden windows excluded). If BarutSRB/OmniWM#349
...
These would lock in nehir's *own* mitigation (not PR BarutSRB/OmniWM#364's clamp):
...
5. **No window is dropped from the diff.** For a window whose computed frame lies outside
   ...
   change — never silence. (This is the regression PR BarutSRB/OmniWM#364's `removeValue` would
   introduce.)
