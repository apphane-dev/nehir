# OmniWM PR BarutSRB/OmniWM#318 — "fix(rules): resolve popup-induced floating bug (BarutSRB/OmniWM#306)" — Discovery
...
discovery. Targets issue BarutSRB/OmniWM#306, which upstream closed as `completed`.
Scope of this doc: determine whether BarutSRB/OmniWM#318's proposed fix applies to nehir and
whether it owns any repo action not already covered by the sibling BarutSRB/OmniWM#323 discovery.
...
> **Filed under discovery/noop/** — BarutSRB/OmniWM#318 is the superseded, internally-broken **first
> iteration** of the BarutSRB/OmniWM#306 (popup-induced floating) fix. Its successor, sibling
> discovery `discovery/20260616-omniwm-323-floating-panel-bar-filter.md`
> (PR BarutSRB/OmniWM#323, same author `@Paranjayy`, same root cause, verdict 🟡 Partial), owns every
> repo action that is actually applicable here. The remainder of BarutSRB/OmniWM#318 is either already
> present in nehir in broader form, or a regressive deletion that would reintroduce the
> bug. Porting BarutSRB/OmniWM#318 would duplicate and regress; no new action is owned.

---

## TL;DR

- **PR BarutSRB/OmniWM#318 is a broken first draft of the BarutSRB/OmniWM#306 fix: its own regression test references a `childWindowStructuralAnchor` rule that the diff never implements, and it deletes the very heuristic and top-level guard (BarutSRB/OmniWM#323 re-adds them) plus drops `parentWindowId` storage. nehir already has the correct versions of everything BarutSRB/OmniWM#318 gets right.**
- **Verdict:** ⚪ **Won't port / not applicable** — duplicate of sibling BarutSRB/OmniWM#323 for the genuinely-applicable parts; the rest is already-fixed-in-nehir or regressive.
...
- CleanShot-gated window-server resolution (a path BarutSRB/OmniWM#318 alone broadens): `Sources/Nehir/Core/Controller/WMController.swift:2047`.
...
2. **`AppAXContext.swift` (+18): replaces** the `shouldTreatAsTopLevelWindow` guard with
   ...
   owned by sibling BarutSRB/OmniWM#323**, which re-adds the same admission while keeping the heuristic.
...
6. **`WMControllerReevaluationRegressionTests.swift` (+123): mode-preservation tests.**
   nehir's equivalent behavior is already covered (sibling BarutSRB/OmniWM#323 cites the nehir
   heuristic/reevaluation tests).

## Why this doesn't apply (it is a duplicate + partly regressive)

Mapping every BarutSRB/OmniWM#318 element to its nehir disposition:

| BarutSRB/OmniWM#318 change | nehir status | Owned by |
|---|---|---|
...
| Admit titled `AXDialog`/`AXPanel` in `AppAXContext` | ⚪ gap exists (`AppAXContext.swift:434`) | **sibling BarutSRB/OmniWM#323** (already owns the eval) |
| Broaden mode-preservation to `fallbackLayout` | 🟢 already broader (`WMController.swift:1920`, source-agnostic) | n/a — already in nehir |
| Drop CleanShot gate on window-server resolution | only consumer (`childWindowStructuralAnchor`) absent from BarutSRB/OmniWM#318's own diff | **sibling BarutSRB/OmniWM#323** (structural-parent eval) |
| Remove `parentWindowId` from metadata | 🟢 nehir keeps it (`WMController.swift:3086`); removal = **regression** | n/a — don't remove |
| `childWindowStructuralAnchor` structural rule | absent in nehir; **also absent from BarutSRB/OmniWM#318's diff** (test-only) | **sibling BarutSRB/OmniWM#323** (as design input) |
| `WMControllerReevaluationRegressionTests` | 🟢 equivalent behavior covered | n/a — already in nehir |

The decisive points:

- **The applicable gaps are BarutSRB/OmniWM#323's, not BarutSRB/OmniWM#318's.** The two genuinely-missing pieces in
  ...
  to this PR (it re-adds the heuristic and guard that BarutSRB/OmniWM#318 wrongly deletes, and it
  actually implements the structural rule BarutSRB/OmniWM#318 only tests for).
- **BarutSRB/OmniWM#318's unique edit (dropping the CleanShot gate, `WMController.swift:2047`) has no
  consumer in BarutSRB/OmniWM#318.** Its only purpose is to feed `parentId` to a `childWindowStructuralAnchor`
  rule that the diff never adds. The successor BarutSRB/OmniWM#323 deliberately omits this broadening,
  and the BarutSRB/OmniWM#323 discovery advises against adopting a blanket `parentId != 0 → floating`
...
- **Three of BarutSRB/OmniWM#318's edits are outright regressive** in nehir (deleting
  `shouldTreatAsTopLevelWindow`, deleting the `.nonStandardSubrole` heuristic, removing
  `parentWindowId` storage). Porting BarutSRB/OmniWM#318 would reintroduce classification gaps and lose
...
**Do not port BarutSRB/OmniWM#318.** It is superseded by the sibling BarutSRB/OmniWM#323 discovery, which already owns
...
BarutSRB/OmniWM#318's mode-preservation, heuristic, top-level guard, and `parentWindowId` storage. Treat
BarutSRB/OmniWM#318 solely as historical context for BarutSRB/OmniWM#306; route any implementation work to the BarutSRB/OmniWM#323
discovery.
