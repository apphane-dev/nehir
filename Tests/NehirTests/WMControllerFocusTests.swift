...
@Suite(.serialized) struct WMControllerFocusTests {
    ...
    @Test @MainActor func focusNotificationsTrackWorkspaceManagerOwnedState() {
        ...
        let focusObserver = center.addObserver(
            forName: .nehirFocusChanged,
            object: controller,
            queue: nil
        ) { notification in
            focusToken.value = notification.userInfo?[NehirFocusNotificationKey.newWindowToken] as? WindowToken
        }
        let workspaceObserver = center.addObserver(
            forName: .nehirFocusedWorkspaceChanged,
            object: controller,
            queue: nil
        ) { notification in
            workspaceIdBox.value = notification
                .userInfo?[NehirFocusNotificationKey.newWorkspaceId] as? WorkspaceDescriptor.ID
        }
        let monitorObserver = center.addObserver(
            forName: .nehirFocusedMonitorChanged,
            object: controller,
            queue: nil
        ) { notification in
            monitorDisplayIdBox.value = notification
                .userInfo?[NehirFocusNotificationKey.newMonitorIndex] as? CGDirectDisplayID
        }
    ...
    @Test @MainActor func managedActivationPublishesCoherentCrossMonitorNotifications() {
        ...
        #expect(fixture.controller.workspaceManager.focusedHandle == secondaryHandle)
        #expect(fixture.controller.workspaceManager.interactionMonitorId == fixture.secondaryMonitor.id)
        #expect(focusInfo.value?[NehirFocusNotificationKey.oldWindowToken] as? WindowToken == primaryHandle.id)
        #expect(focusInfo.value?[NehirFocusNotificationKey.newWindowToken] as? WindowToken == secondaryHandle.id)
        #expect(focusInfo.value?[NehirFocusNotificationKey.oldHandleId] as? WindowToken == primaryHandle.id)
        #expect(focusInfo.value?[NehirFocusNotificationKey.newHandleId] as? WindowToken == secondaryHandle.id)
        #expect(workspaceInfo.value?[NehirFocusNotificationKey.oldWorkspaceId] as? WorkspaceDescriptor.ID == fixture
            .primaryWorkspaceId)
        #expect(workspaceInfo.value?[NehirFocusNotificationKey.newWorkspaceId] as? WorkspaceDescriptor.ID == fixture
            .secondaryWorkspaceId)
        #expect(monitorInfo.value?[NehirFocusNotificationKey.oldMonitorIndex] as? CGDirectDisplayID == fixture
            .primaryMonitor.displayId)
        #expect(monitorInfo.value?[NehirFocusNotificationKey.newMonitorIndex] as? CGDirectDisplayID == fixture
