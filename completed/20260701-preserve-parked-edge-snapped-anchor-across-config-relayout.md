...
`centeredViewportCorrection` block). The implementation lands the config/parked-snap
slice of that shared Phase 3 behavior.
...
- **Fix.** Gate both blocks on an actual change that justifies a reveal/recenter,
  ...
  and `revealForFocusActivation` (which already no-ops on fully visible). Preserve
  reachable parked snap points when no such change occurred.

## Status

- Root cause source-attributed (see discovery); no further capture was required.
- Implemented in `Sources/Nehir/Core/Controller/NiriLayoutHandler.swift` by:
  - remembering `columnIdsBeforePass` before removal/sync work;
  - deriving `selectionOrLayoutChanged` from selection, active-column, column-id, and removal visibility changes;
  - skipping multi-column `ensureSelectionVisible` on pure config/settings relayouts with no real selection/layout change;
  - allowing `centeredViewportCorrection` only when a real change happened or the current view start is no longer a reachable snap.
- Regression coverage added in `Tests/NehirTests/ParkedViewportRelayoutTests.swift` for unchanged config relayouts, width changes that still preserve reachable parked snaps, and width changes that legitimately recenter once the previous parked snap becomes invalid.
...
### Phase 1 — Gate Block 1 (`ensureSelectionVisible`) on an actual change — done
...
### Phase 2 — Preserve edge-snaps in Block 2 (`centeredViewportCorrection`) — done
...
### Phase 3 — Regression test — done

Coverage added after implementation in `Tests/NehirTests/ParkedViewportRelayoutTests.swift`:
...
The tests use a synthetic multi-column fixture in `Tests/NehirTests/ParkedViewportRelayoutTests.swift`.
...
- User-visible bug fix changeset added on the implementation branch.
- Coordinates with `planned/20260625-unrecorded-viewport-offset-mutation-attribution.md`
  Phase 3 by landing the shared multi-column relayout change gate plus the
  edge-snap-specific `centeredViewportCorrection` preservation.
