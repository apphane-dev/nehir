...
- Implementation: `SideHiding.placement` (the scroll-hide render + park target) uses
  `parkingFrame = monitor.visibleFrame` with a 1pt reveal, so render and park targets both
  derive from `placement` and never disagree. `physicalScreenEdgeOrigin`
  (workspace-inactive / scratchpad hides) intentionally keeps `parkingFrame = monitor.frame`
  — the true physical screen edge — as its inline comment states.
