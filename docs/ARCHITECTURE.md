...
**Column width presets** cycle through configurable proportions (default: 1/3, 1/2, 2/3). Proportional spans are resolved as `(availableSpace - gap) * proportion - gap`; a contiguous group whose proportions sum to `1.0` therefore fits the viewport under Nehir's gap accounting without users compensating for gaps. Full-width mode expands a column to fill the monitor.
