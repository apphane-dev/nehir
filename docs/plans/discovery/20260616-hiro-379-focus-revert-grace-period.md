# Hiro PR #379 — "Fix focus reverting on rapid next/prev (→#317)" — Discovery

Source PR: https://github.com/BarutSRB/Hiro/pull/379 — "Fix focus direction race on
rapid next/prev key presses", targets issue #317.
Companion issue doc: `docs/plans/discovery/20260616-hiro-317-rapid-focus-revert-race.md`
(verdict 🔴 Open — owns the root-cause analysis; **this doc does not re-derive the race**).
Filed against: `BarutSRB/Hiro` (upstream of nehir — `NOTICE.md` states nehir is a fork
of `BarutSRB/OmniWM`, renamed to Hiro; module path `Sources/OmniWM/` → `Sources/Nehir/`).
...
`github.com/BarutSRB/Hiro/pull/379.diff` (HTTP 200); nehir symbols were re-read live.

> **Consolidation note.** This is the single authoritative discovery for PR #379. It
> supersedes two near-duplicate #379 drafts the discovery orchestrator emitted in the
> same run — `20260616-hiro-379-focus-revert-300ms-grace-pr.md` and
> `20260616-hiro-379-managed-request-grace-period.md` (same verdict, same HEAD, same
...
- **PR #379 stamps the *pending* `ManagedFocusRequest` with a creation time and makes the
  ...
  self-echo?" check, but only on the *confirmed* path; PR #379 adds the *pending*-path
...
- **Verdict:** 🟡 **Partial — port the *concept* adapted, never the diff verbatim.**
  ...
  self-echoes) on the *confirmed* path; PR #379 adds the *pending*-path half that nehir
...
## Merge state — inherited from the #317 doc (API rate-limited at re-check)

The catalog triage note calls PR #379 **"open."** The #317 doc records a live GitHub
...
| `closed_at` | `2026-06-15T23:35:07Z` (3 min after issue #317 was closed-completed) |
| `head.repo` / `head.ref` | `biswadip-paul/OmniWM` / `fix/focus-direction-race-on-rapid-keypress` |

I attempted to re-verify live; the unauthenticated `/repos/BarutSRB/Hiro/pulls/379`
endpoint is currently rate-limited (returns no fields). I therefore **inherit the #317
...
`AXEventHandler.swift:93`/`:107`/`:3633`/`:3687` — placement, not focus). `NOTICE.md`
confirms the OmniWM→Hiro lineage.
...
the #317 doc identifies: a pending request is, by definition, not yet confirmed, so
...
| nehir line (HEAD `7f61cb3`) | Enclosing function | PR hunk (upstream line) | Patched by #379? |
|---|---|---|---|
...
*confirmed* path.** PR #379 adds the missing *pending*-path half.
...
exact question the PR answers for the *pending* path. The #317 race lives entirely on
the pending path, where `ManagedFocusRequest` has no timestamp; that is the gap PR #379
...
3. **Dual-window gap (0.3 s vs 0.6 s).** nehir gates the confirmed-path echo recognition
   on **0.6 s** (`nativeAppSwitchLeaseRequestConfirmationGrace`, `:314`). PR #379
   ...
   pending path — a residual revert window the two disagree on. The #317 doc: *"use 0.6 s
...
4. **Time heuristic, not echo classification (why upstream abandoned it).** The gate's new
   ...
   stale echo arriving **after** 300 ms still reverts. Hiro `main` replaced the timer with
   `IntentLedger.classifyFocusObservation → .echoOf/.lateEcho/.external` (verified in
   `main` at `AXEventHandler.swift:3680`, quoted in the #317 doc) and deleted
   `FocusBridgeCoordinator`. `main` has **no** `managedRequestGracePeriod`/`createdAt`/
   `age`. PR #379 is the *rejected* branch, not the shipped fix.

## Recommendation

🟡 **Do not apply PR #379 verbatim — use it as design provenance, and port the *concept*
adapted.** That adaptation is exactly "Option B" in the #317 doc:
...
**Long-term:** plan the `IntentLedger` rewrite (the #317 doc's Option C) — the unifying
primitive for this race and the structurally-parallel FFM focus race
(`docs/plans/discovery/20260616-ffm-steals-focus-behind-overlay-on-stale-queued-mouse-moves.md`).
PR #379's timer is a fast stopgap; it should not be the terminal design.

## Suggested tests

The full regression set lives in the #317 doc (rapid focus-next settles on the final
...
Note (per #317 doc): three existing tests encode the *pre-fix* contract and will flip red
...
#317 doc; **re-verify at HEAD**). A port is not "just add tests"; these must be qualified.
