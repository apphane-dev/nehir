# Hiro issue #301 — "3 finger swiping gets stuck on windows" — Discovery

Source issue: <https://github.com/BarutSRB/Hiro/issues/301>
...
> diagnostic gap). Hiro #301 is the 3-finger / app-competition manifestation
...
> already prescribe. Porting the upcoming Hiro fix directly is N/A — nehir
> does not share the Hiro rewrite's code (nehir forks OmniWM).
...
Reported 2026-05-06 against Hiro `0.4.8.1`, still **open**. Title: "3 finger
...
- **`nick-s5` (2026-06-15):** *"The issue … seems to have to do with whatever
  ...
  then that seems to take precedence over Hiro's 3 finger swipe detection,
...
that also consumes 3-finger touchpad input interferes with nehir/Hiro's
...
- nehir-53 Recommendation #1 (trace the abort/skip path with a `reason`) and #3
  ...
  directly address Hiro #301's 3-finger "stuck/glitch": the hysteresis absorbs
...
> Sibling reference in triage: Hiro #336 ("gesture scroll without scroll-snap")
...
"fix tomorrow" Hiro change — nehir does not share the Hiro rewrite's code
(nehir forks OmniWM per `NOTICE.md`), and the relevant logic already lives in
