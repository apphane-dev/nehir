# Hiro issue #233 — "Center focused column 'on overflow' triggers with 2 50% sized windows" — Discovery

Source issue: <https://github.com/BarutSRB/Hiro/issues/233>
...
> is not luck: the entire Hiro mechanism that produced it
...
> (`docs/plans/completed/20260612-viewport-navigation-redesign.md`). The replacement
...
- **The Hiro setting that had this bug, `centerFocusedColumn = "onOverflow"`, does not
  ...
  are in `.changeset/` and `docs/plans/` migration history).
...
- **The Hiro #233 scenario is an existing, passing regression test.**
...
Yes in lineage, **no in mechanism.** nehir forks OmniWM (renamed Hiro); see `NOTICE.md`.
The symbols #233 names — the `centerFocusedColumn`/`"onOverflow"` setting and the
...
- A repo-wide search for `centerFocusedColumn | CenterFocusedColumn | onOverflow |
  ...
  the (completed) redesign plan `docs/plans/completed/20260612-viewport-navigation-redesign.md`.
- The redesign's Task 9 deleted them explicitly:
  ...
  (`docs/plans/completed/20260612-viewport-navigation-redesign.md:248`) and
...
Filed 2026-04-11 on Hiro (≈0.4.7.x) by `Guria`. With **"Center Focused Column" set to
...
(a Hiro-specific quirk, not the nehir behavior — see below).
...
The removed Hiro setting maps onto nehir's new model one-to-one:
...
So the Hiro user in #233 (on `onOverflow`) is, in nehir, a `revealPartial = .default`
...
The `2 * gap + pixelTolerance` tolerance is exactly what makes the Hiro reporter's
...
behavior BarutSRB said he wanted but shipped broken in the Hiro `onOverflow` mode ("clamp
...
| #233 ingredient (Hiro) | nehir state |
|---|---|
...
The Hiro reporter's secondary observation — that the `-0.01` workaround *also* suppressed
centering on genuinely overflowing windows — is a Hiro-only artifact of how that build
...
2. **Nothing to port against.** The host mechanism (`centerFocusedColumn` +
   ...
   (`docs/plans/completed/20260612-viewport-navigation-redesign.md:248,252`). A direct
...
3. **Already guarded + tested.** nehir's `scrollToReveal` `.fullyVisible` + `fillsViewport`
   ...
   layer than the Hiro "onOverflow" heuristic, and the regression test above locks it in.
...
gap-tolerance that makes the Hiro `-0.01` workaround unnecessary, a parametrized variant is
straightforward:
