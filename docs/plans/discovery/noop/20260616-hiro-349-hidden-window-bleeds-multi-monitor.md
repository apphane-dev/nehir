# Hiro issue #349 — "Hidden window bleeds into view (multi-monitor)" — Discovery

Source issue: https://github.com/BarutSRB/Hiro/issues/349
Proposed fix (upstream PR): https://github.com/BarutSRB/Hiro/pull/364 — "Clamp visible
...
`noop/20260616-hiro-235-window-bleed-different-workspace.md`) and #364. This doc keeps to
...
- `Sources/Nehir/Core/Layout/Niri/NiriLayoutEngine+Animation.swift:228` — nehir's
  `calculateCombinedLayoutUsingPools` is the 1:1 counterpart of Hiro's
...
```swift
// PR #364 — verbatim from https://github.com/BarutSRB/Hiro/pull/364.diff
...
So, as in pre-fix Hiro, a tiled frame is never intersected with monitor bounds *after*
...
2. **The hidden-window strip (the literal report) is handled by a different path.**
   ...
   PR #364 explicitly **excludes** `hiddenPool` windows, so even in Hiro it cannot address
