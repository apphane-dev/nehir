# Hiro PR #401 — "Dwindle split orientation respects per-monitor settings (→#270)" — Discovery

Source PR: https://github.com/BarutSRB/Hiro/pull/401
Merge state: **closed without merge** (`state: closed`, `merged: false`, `merged_at: null`).
Related issue: Hiro #270 (see sibling discovery
`docs/plans/discovery/noop/20260616-hiro-270-dwindle-third-window-outside-grid-overlap.md`).
...
> **Filed under `discovery/noop/`** — PR #401 owns no new nehir action. It targets Hiro's
...
PR #401 claims Hiro's Dwindle split orientation used the global `settings.splitWidthMultiplier`
...
Lineage is real: `NOTICE.md:3-7` says nehir is a fork of BarutSRB's OmniWM, and that OmniWM was
renamed to Hiro. But the code family patched by PR #401 is not present in this fork:
...
The name `summonWindowRight` exists in nehir, but it is a Niri-column operation, not Hiro's Dwindle
