# Hiro PR #384 — "Respect window min-size constraints in Niri column width" — Discovery

Source PR: https://github.com/BarutSRB/Hiro/pull/384
Author: @biswadip-paul (co-authored by "Claude Opus 4.6"); one source commit + tests, single file.
Fixes issue: https://github.com/BarutSRB/Hiro/issues/383 (related to #268/#283).
Filed against: `BarutSRB/Hiro` (upstream of nehir — see `NOTICE.md`; nehir is a fork of
`BarutSRB/OmniWM`, which was renamed to Hiro).
...
> **does not exist in nehir**. nehir never adopted Hiro's `resizePlaceholderState` subsystem;
...
> `20260616-hiro-403-frame-write-race-min-size-suppression.md`; see the final section for why
...
```swift
// Hiro LayoutRefreshController.swift (snapshot build loop)
...
- nehir's snapshot-build loop is `buildWindowSnapshots` at
  `Sources/Nehir/Core/Controller/LayoutRefreshController.swift:482` (1:1 role with Hiro's site).
...
Hiro's bug is `mergedConstraints.relaxedForResizePlaceholder()` applied to **every** window,
...
The triage note pairs this PR with `20260616-hiro-403-frame-write-race-min-size-suppression.md`
