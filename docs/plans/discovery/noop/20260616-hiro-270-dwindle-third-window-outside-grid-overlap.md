# Hiro issue #270 — "Dwindle window placement/sizing" — Discovery

Source issue: https://github.com/BarutSRB/Hiro/issues/270
Related PR (assessed, not assumed as the fix): https://github.com/BarutSRB/Hiro/pull/401
...
Lineage itself is confirmed: `NOTICE.md:3-7` states nehir is a fork of `BarutSRB/OmniWM`, renamed
to Hiro. So #270 is genuinely upstream-of-nehir; it is simply that nehir never adopted (or has
already shed) the Dwindle layout. The Dwindle engine is Hiro-specific surface area that nehir does
not carry.
...
| Symbol (Hiro #401 / engine) | Present in nehir? |
|---|---|
...
| `summonWindowRight` (Hiro Dwindle path) | ❌ absent in this form |
...
tabbed mode) is owned by the separate `docs/plans/completed/20260615-new-window-placement-*`
...
3. (Cross-doc) If a future discovery reports a real nehir "3rd window mis-placed/overlapping"
   ...
   `20260616-hiro-384-…`, `20260616-hiro-403-…`), **not** here — nehir's placement model is
