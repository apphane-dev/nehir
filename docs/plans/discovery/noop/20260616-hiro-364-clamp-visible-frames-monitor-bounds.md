# Hiro PR #364 — "Fix windows overlapping across monitors" (→#349) — Discovery

Source PR: https://github.com/BarutSRB/Hiro/pull/364
Author: @biswadip-paul (co-authored by "Claude Opus 4.6"); two commits, +23 −0, single file.
Targets issue: https://github.com/BarutSRB/Hiro/issues/349 (discovered in parallel →
`docs/plans/discovery/20260616-hiro-349-hidden-window-bleeds-multi-monitor.md`).
Filed against: `BarutSRB/Hiro` (upstream of nehir — see `NOTICE.md`; nehir is a fork of
`BarutSRB/OmniWM`, renamed to Hiro).
...
> `20260616-hiro-364-clamp-visible-frames-post-layout-noop.md` (an independent re-verification
> pass) and `20260616-hiro-364-clamp-visible-frames-to-monitor-bounds.md`. This doc subsumes
> their content.
...
"Closed without merge"). So #364 was **rejected upstream** — it never landed in Hiro. That
...
- `Sources/Nehir/Core/Layout/Niri/NiriLayoutEngine+Animation.swift:228` —
  ...
  hiddenPool)`. This is 1:1 with Hiro's
...
```swift
// Hiro: NiriLayoutEngine+Animation.swift — inserted just before `return (framePool, hiddenPool)`
...
nil`), so even in Hiro the patch cannot address the reporter's literal symptom.
...
| | Hiro #349 / PR #364 | nehir stale-live-frame discoveries |
|---|---|---|
