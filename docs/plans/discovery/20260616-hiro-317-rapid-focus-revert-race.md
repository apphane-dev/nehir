# Hiro issue #317 — "Rapid focus-next/prev snaps back to the wrong window" — Discovery

Source issue: https://github.com/BarutSRB/Hiro/issues/317 — "When moving focus to
next/prev quickly, focus randomly goes in opposite direction"
Companion PR: https://github.com/BarutSRB/Hiro/pull/379 — "Fix focus reverting on
rapid next/prev key presses (→#317)"
Filed against: `BarutSRB/Hiro` (upstream of nehir — `NOTICE.md` states nehir is a
fork of `BarutSRB/OmniWM`, renamed to Hiro). Scope: determine whether the
...
- **The upstream fix is not directly portable.** PR #379 (300 ms grace period) was
  **closed without merge**; Hiro `main` instead shipped a large architectural
...
two **contributor forks** (not `BarutSRB/Hiro`): the unmerged PR #379 branch above,
...
`NOTICE.md` confirms the OmniWM→Hiro lineage.
...
different actor and path. Hiro `main`'s `IntentLedger` was built to solve exactly
