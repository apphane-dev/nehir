...
enum AppCLIInstallError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        ...
        case let .bundledCLIMissing(url):
            return "Bundled nehirctl was not found at \(url.path). Reinstall Nehir to restore the CLI."
        case let .conflictingInstall(url):
            return "Another file already exists at \(url.path). Move or remove it before installing Nehir's CLI link."
        case let .cannotRemoveNonManagedInstall(url):
            return "Nehir will only remove the CLI link it created. The existing file at \(url.path) is managed elsewhere."
        }
    }
}
...
final class AppCLIManager {
    ...
    init(
        ...
        homebrewLinkURLsProvider: @escaping () -> [URL] = {
            [
                URL(fileURLWithPath: "/opt/homebrew/bin/nehirctl"),
                URL(fileURLWithPath: "/usr/local/bin/nehirctl")
            ]
        }
    ) {
    ...
    var bundledCLIURL: URL {
        bundleURLProvider()
            ...
            .appendingPathComponent("nehirctl", isDirectory: false)
    }
