...
enum CLIParser {
    ...
    static let usageText: String = {
        ...
        lines += ruleLines.map { "  nehirctl \($0)" }
        lines += [
            "  nehirctl query <\(queryNames)> [selectors...] [--fields <csv>] [--format <json|table|tsv|text>]"
        ]
        lines += workspaceLines.map { "  nehirctl \($0)" }
        lines += windowLines.map { "  nehirctl \($0)" }
        lines += [
            "  nehirctl subscribe <\(subscriptionNames)> [--no-send-initial]",
            "  nehirctl subscribe --all [--no-send-initial]",
            "  nehirctl watch <\(subscriptionNames)> [--no-send-initial] --exec <argv...>",
            "  nehirctl watch --all [--no-send-initial] --exec <argv...>",
            "",
            "Formats:",
