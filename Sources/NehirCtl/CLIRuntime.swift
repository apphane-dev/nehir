...
enum CLIRuntime {
        ...
        do {
            ...
            if isTransportError(error) {
                writeLocalFailure(
                    ...
                    fallbackMessage: "nehirctl: \(error)"
                )
            ...
            do {
                ...
                case let .childLaunch(underlying):
                    writeLocalFailure(
                        ...
                        code: .internalError,
                        exitCode: .internalError,
                        fallbackMessage: "nehirctl: \(underlying)"
                    )
                    return CLIExitCode.internalError.rawValue
                }
            } catch {
    ...
    private static func reportWatchChildFailure(result: WatchChildResult, command: [String]) {
        ...
        case .exit:
            message = "nehirctl watch: child exited with status \(result.terminationStatus): \(commandText)\n"
        case .uncaughtSignal:
            message = "nehirctl watch: child terminated by signal \(result.terminationStatus): \(commandText)\n"
        case .unknown:
            message = "nehirctl watch: child terminated unexpectedly: \(commandText)\n"
        }
