...
- **Update 2026-07-10:** the arming path is now identified — quick-terminal summon arms the flag with the managed token preserved, and the CR-1 close-churn suppression swallows the only clearing confirmation. See [`20260710-ghostty-quick-terminal-arms-stale-nonmanaged-focus.md`](20260710-ghostty-quick-terminal-arms-stale-nonmanaged-focus.md) for repro steps and fix slices; a clearing fix at the suppression point (or overlay-liveness-tied clearing) now looks like the highest-leverage NF-1 change, ahead of resolver-side escapes.
...
- [`20260710-ghostty-quick-terminal-arms-stale-nonmanaged-focus.md`](20260710-ghostty-quick-terminal-arms-stale-nonmanaged-focus.md) — **arming mechanism + exact repro steps.** Complete arm → stuck → re-arm → still-stuck capture: `command_target.resolve.decline reason=nonManagedFocus.frontmostTracked` fires while `frontmostToken == confirmedToken == layoutSelectionToken` (all the same tracked managed window), and the state persists across a fresh quick-terminal summon and a full overlay destroy. Includes observed recovery via `focusNextWindow` and three candidate fix slices.
...
- [`20260710-quick-terminal-close-after-restart-lacks-overlay-evidence.md`](20260710-quick-terminal-close-after-restart-lacks-overlay-evidence.md) — **open cold-start boundary condition (2026-07-10).** All overlay/close evidence (`overlayCapablePids`, recent non-managed focus, recent same-app close, live overlay scan) is in-memory and observation-driven; when Nehir restarts while the quick terminal is already open, the first QT close evaluates with every evidence field false and close recovery reveals the parked Ghostty column. Not a regression of `d3ef41ee` or `5d34d706` — the guards behave as designed on empty inputs. Fix slice: populate `overlayCapablePids` from overlay window-rule decisions (the startup full refresh already classifies the open QT).
...
**Current state:** the known CR-1 bounce (A→B→A oscillation) is fixed. The 2026-07-09 reopen (quick-terminal close scroll/workspace-switch) is **landed and fixed** as of `d3ef41ee`. **Known NF-1 side effect (2026-07-10):** the `d3ef41ee` overlay-close-churn suppression also discards the managed activation that would clear stale non-managed focus after quick-terminal dismissal — see [`20260710-ghostty-quick-terminal-arms-stale-nonmanaged-focus.md`](20260710-ghostty-quick-terminal-arms-stale-nonmanaged-focus.md). Any NF-1 fix there must preserve this cluster's viewport-stillness behavior. The adjacent cross-app successor-activation finding above remains **open** but is a distinct root cause (successor selection, not same-app overlay recovery) — track it separately rather than folding it into CR-1's reopen criterion. Otherwise, do not reopen this cluster for generic non-managed-focus command failures (NF-1) or generic viewport reveal movement (VR-1) unless new evidence again shows same-app close/overlay recovery redirects choosing opposite stable targets.
