...
- [`20260709-quick-terminal-long-open-close-reveals-parked-ghostty-viewport.md`](20260709-quick-terminal-long-open-close-reveals-parked-ghostty-viewport.md) — **reopens CR-1 (2026-07-09).** Quick-terminal close acts on macOS's same-app focus redirect to a managed Ghostty window on the origin workspace. Variant A: on the active origin workspace the reveal loses the destroy-vs-redirect arming race and scrolls to the parked column (also VR-1). Variant B: on an *inactive* origin workspace `close_recovery_overlay_stable_target` refocuses there and switches the active workspace back — `redirectToStableSameAppRecoveryFocusIfNeeded` (`AXEventHandler.swift:2669`) has no active-workspace guard.

**Current state:** the known CR-1 bounce (A→B→A oscillation) is fixed. **Reopened 2026-07-09** by `20260709-quick-terminal-long-open-close-reveals-parked-ghostty-viewport.md`: new evidence shows the overlay stable-recovery redirect firing against an inactive origin workspace and the active-workspace reveal losing the arming race — both are same-app close/overlay recovery redirects disturbing viewport/workspace, matching the reopen criterion below. Otherwise, do not reopen this cluster for generic non-managed-focus command failures (NF-1) or generic viewport reveal movement (VR-1) unless the new evidence again shows same-app close/overlay recovery redirects choosing opposite stable targets.
