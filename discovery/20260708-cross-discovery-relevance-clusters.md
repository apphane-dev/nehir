...
- [`20260708-stale-nonmanaged-focus-suppresses-managed-selection-and-window-move.md`](20260708-stale-nonmanaged-focus-suppresses-managed-selection-and-window-move.md) — current strongest capture: viewport selection moves to visible managed windows, but stale non-managed focus keeps `confirmedManagedFocusToken` old and `wmCommandTarget=nil`.
...
- [`20260708-cold-start-destroy-only-replacement-bursts-click-readmit.md`](20260708-cold-start-destroy-only-replacement-bursts-click-readmit.md) — newer captures with the same cold-start class but stronger managed-replacement evidence: windows are initially AX-enumerated and inserted, destroy-only replacement bursts remove them, and the active workspace is rebuilt only through focus/pid recovery. A later clean start shows the worse variant: the first eight-column layout collapses, Code/Helium/Slack/Claude recover piecemeal, and Ghostty remains visible but unmanaged.
