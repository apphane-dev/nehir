...
- [`20260708-cold-start-destroy-only-replacement-bursts-click-readmit.md`](20260708-cold-start-destroy-only-replacement-bursts-click-readmit.md) — newer `d4cc52` capture with the same cold-start class but stronger managed-replacement evidence: all eight windows were initially AX-enumerated and inserted, destroy-only replacement bursts removed them, and Telegram/Claude re-entered only after `workspaceDidActivateApplication` clicks while Helium stayed unmanaged.
