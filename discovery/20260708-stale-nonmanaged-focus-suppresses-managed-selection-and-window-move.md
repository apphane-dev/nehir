...
- [`../completed/20260710-ghostty-quick-terminal-arms-stale-nonmanaged-focus.md`](../completed/20260710-ghostty-quick-terminal-arms-stale-nonmanaged-focus.md) answered this discovery's open question of **where the staleness comes from**: quick-terminal summon arms the flag with the managed token preserved, and the overlay-close-churn suppression (shipped for CR-1 in `d3ef41ee`) discarded the only activation whose confirmation would clear it. **Landed** in `31c8b851`.
- [`20260708-assign-to-workspace-7-nonmanaged-command-target-decline.md`](20260708-assign-to-workspace-7-nonmanaged-command-target-decline.md) is the direct post-instrumentation follow-up: `command_target.resolve.decline reason=nonManagedFocus.frontmostTracked` proves why workspace-7 assignment attempts had no command target until managed focus recovered.
