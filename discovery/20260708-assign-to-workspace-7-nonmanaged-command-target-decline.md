...
- [`../completed/20260710-ghostty-quick-terminal-arms-stale-nonmanaged-focus.md`](../completed/20260710-ghostty-quick-terminal-arms-stale-nonmanaged-focus.md) supplied the arming mechanism this note lacked: a 2026-07-10 capture shows the same `nonManagedFocus.frontmostTracked` decline firing **with** `confirmedToken` and `layoutSelectionToken` populated (all equal to the frontmost tracked token) and never self-recovering, after a Ghostty quick-terminal summon/dismiss armed the flag and close-churn suppression swallowed the clearing confirmation. **Landed** in `31c8b851`.
